...
public final class CardPanel extends StackPane {
    ...
    public CardPanel() {
        ...
        final VBox deckUnderlay = buildDeckUnderlay();

        setAlignment(Pos.TOP_CENTER);
        getChildren().addAll(deckUnderlay, cardVisual);
    }
    ...
    private VBox buildCardVisual() {
        ...
        final StackPane overlay = new StackPane(characterSlot, decisionHint, tl, tr, bl, br);
        card.getChildren().add(overlay);
        return card;
    }

    private VBox buildDeckUnderlay() {
        final VBox underlay = new VBox();
        underlay.setMaxSize(CARD_W, CARD_H);
        underlay.setStyle(parchmentStyle(COLOR_PARCHMENT_DARK));
        underlay.setTranslateY(DECK_OFFSET);
        return underlay;
    }
    ...
    private void showHint(final double offsetX) {
        ...
        decisionHint.setOpacity(Math.min(Math.abs(offsetX) / DRAG_THRESHOLD, 1.0));
