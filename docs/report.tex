...
Il costruttore applica la Dependency Injection ricevendo \texttt{GameConfig} e \texttt{Deck} come argomenti, e crea internamente il \texttt{GameClockImpl}:
\begin{lstlisting}
...
\end{lstlisting}
Il terminale \texttt{orElseGet} esegue il fallback solo se lo stream è vuoto (tutte le carte sono state utilizzate), restituendo la prima carta come comportamento di default.
