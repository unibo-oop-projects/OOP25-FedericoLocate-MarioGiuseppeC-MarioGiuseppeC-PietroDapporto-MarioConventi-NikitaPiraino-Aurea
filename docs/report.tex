...
\begin{itemize}
    ...
    \item Sistema di carte figlie: alcune decisioni 
...
\begin{itemize}
    \item Visualizzazione proporzionale dell'impatto della carta corrente 
          tramite indicatori visivi sulle icone dei parametri (pallini 
          di anteprima durante il trascinamento).
    \item Modalit\`a ``apocalittica'' con eventi più fantasiosi.
    \item Easter egg ambientati nel contesto Unibo.
\end{itemize}

\paragraph{Requisiti non funzionali}
\begin{itemize}
    \item L'applicazione deve essere distribuita come singolo file JAR 
          eseguibile su qualunque piattaforma con Java~21.
    \item L'interfaccia grafica deve essere reattiva e gradevole, con 
          ...
          aggiornamento parametri, overlay di fine partita).
    \item Il software deve poter essere avviato senza configurazioni 
          esterne, partendo dal solo JAR.
    \item Il codice deve rispettare le regole di qualità statica 
          (Checkstyle, PMD, SpotBugs) verificate automaticamente 
          a ogni build Gradle.
\end{itemize}
...
% -----------------------------------------------------------------------------
Le entità principali del dominio sono il Rettore (il giocatore),
che gestisce un insieme di \emph{Parametri} dell'Ateneo. Ogni parametro
...
specifica quanto e in quale direzione un parametro viene modificato.

Alcune decisioni attivano un meccanismo di \emph{Conseguenza}: la scelta
effettuata pianifica la comparsa di una carta figlia dopo un numero
definito di turni. La relazione tra carta padre, tipo di scelta e carta
figlia è descritta da un \emph{FollowUp}.
...
turni ciascuno. Ad ogni istante, lo \emph{Stato di gioco} descrive
l'andamento della partita: in corso (\emph{running}), vinta
(\emph{won}) o persa (\emph{lost}).

Prima di iniziare, il giocatore fornisce la propria
\emph{Identità} (nome, facoltà e livello di difficoltà).
La \emph{Difficoltà} influenza il peso degli effetti applicati
ai parametri e la probabilità con cui vengono selezionate le
carte di supporto.
...
conseguenze programmate dalle decisioni precedenti (follow-up). Un secondo
aspetto impegnativo riguarda la reattività dell'interfaccia: la View deve
aggiornarsi automaticamente a ogni variazione dei parametri, senza che il
Model conosca l'esistenza della View. Infine, la gestione del ciclo di vita
dell'applicazione (login, partita, fine partita, riavvio) richiede una
coordinazione precisa fra le componenti senza introdurre dipendenze circolari.
...
\begin{itemize}
    \item Il \textbf{Model} (package \texttt{it.unibo.aurea.model})
        racchiude la logica di gioco e lo stato dell'Ateneo.
        Il punto di ingresso è l'interfaccia \texttt{GameEngine},
        che orchestra \texttt{GameConfig} (configurazione della partita),
        \texttt{GameClock} (progressione temporale), \texttt{Deck}
        (mazzo di carte) e i quattro \texttt{Parameter} (stato
        dell'Ateneo). Il Model non conosce né la View né il Controller.

    \item Il \textbf{Controller} (package \texttt{it.unibo.aurea.controller})
        coordina il flusso di interazione tramite l'interfaccia
        \texttt{GameController}. Riceve le scelte del giocatore dalla
        View (\texttt{makeDecision()}), le inoltra al Model e aggiorna
        la View con il nuovo stato. Gestisce inoltre la connessione
        reattiva fra Model e View registrando gli observer sui parametri.

    \item La \textbf{View} (package \texttt{it.unibo.aurea.view})
        implementa l'interfaccia \texttt{GameView} e presenta
        all'utente lo stato del gioco. Non accede mai direttamente
        al Model: riceve aggiornamenti tramite il pattern Observer
        (notifiche sui parametri) e tramite chiamate esplicite
        del Controller (\texttt{displayCard()}, \texttt{updateTime()},
        \texttt{showVictory()}, ecc.).
\end{itemize}
...
La scelta di MVC garantisce che: (i)~il Model sia completamente indipendente
dalla libreria grafica utilizzata; (ii)~la View possa essere sostituita in
blocco --- passando per esempio da JavaFX a un'interfaccia testuale ---
senza modificare Model o Controller; (iii)~l'aggiunta di nuove funzionalità
possa essere realizzata intervenendo soltanto sui componenti effettivamente
coinvolti.
...
\textbf{Soluzione} \\
In una prima fase sono state considerate diverse soluzioni basate sulla costruzione diretta degli oggetti nel codice, ad esempio tramite il costruttore di \texttt{CardImpl}, tramite factory method o attraverso il pattern Builder (che in parte comunque è stato mantenuto). 
...
In questo modo il model riceve oggetti già costruiti e coerenti con la logica del gioco.
...
\textbf{Pattern utilizzati} \\
In questa parte è stato applicato il pattern Builder all'interno del metodo \texttt{toCard} nella classe \texttt{Deck}. In particolare questo metodo che si occupa di convertire ogni carta da \texttt{CardDTO} a \texttt{Card}. 
Il pattern in questo caso rende più chiara la creazione delle nuove carte con i relativi dati.
...
\begin{figure}[H]
    ...
    \fbox{\parbox[c][4cm][c]{0.85\linewidth}{\centering
        \textit{UML Observer: ParameterImpl (observable),
        ParameterObserver (interfaccia), GameViewJavaFXImpl (observer
        via method reference). Da inserire.}}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][4cm][c]{0.85\linewidth}{\centering
        \textit{UML ISP: ParameterView (sola lettura),
        Parameter estende ParameterView aggiungendo modify(),
        ParameterImpl implementa entrambe. Da inserire.}}}
...
\begin{figure}[H]
    ...
    \fbox{\parbox[c][3cm][c]{0.85\linewidth}{\centering
        \textit{UML ParameterType: enum con campo displayName
        e metodo getDisplayName(). Da inserire.}}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][4cm][c]{0.85\linewidth}{\centering
        \textit{UML Strategy: CardPanel con i tre campi funzionali
        (BiConsumer, Function, Runnable) iniettati da
        GameViewJavaFXImpl. Da inserire.}}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][4cm][c]{0.85\linewidth}{\centering
        \textit{UML IoC: Main fornisce Runnable onRestart a
        GameViewJavaFXImpl; EndgameOverlay lo invoca tramite
        la View senza conoscere Main. Da inserire.}}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][3cm][c]{0.85\linewidth}{\centering
        \textit{UML PlayerInfo: record con tre campi immutabili
        e costruttore compatto con validazione. Da inserire.}}}
    \caption{Value Object PlayerInfo come Java record.}
...
Orchestratore logica, gioco & GameEngine & GameEnginelmpl \\ \hline
...
\subsubsection{Orchestrazione del modello in GameEnginelmpl}
...
\noindent\textbf{Soluzione} \texttt{GameEnginelmpl} implementa \texttt{GameEngine} ed è la classe centrale del sottosistema model. Coordina tre dipendenze: la configurazione (\texttt{GameConfig}), l'orologio (\texttt{GameClock}) e il mazzo (\texttt{Deck}).
...
si gestiscono parallelamente pi`u partite).
\end{itemize}
...
\noindent\textbf{Soluzione:} Il metodo \texttt{getGameState()} nella classe \texttt{GameEnginelmpl} avvia una processo di identificazione dello stato, che poi restituirà uno dei valori dell'enumerazione GameState: RUNNING, WON, LOST.
...
\begin{itemize}
    ...
    \item nelle condizioni non richiamare delle funzioni ma mettere il loro contenuto direttamente nella metodo \texttt{GetGameState()}: supporta meno per necessità future i principi Don't Repeat Yourself e Single Responsability.
\end{itemize}
...
\noindent\textbf{Soluzione} La soluzione adottata separa la lettura dalla scrittura attraverso due interfacce distinte, ripettando pure l'Interface Segregation Principle:
...
\item restituire una struttura dati contenente i valori dei parametri, sarebbe statauna soluzione semplice, però non avrebbe permesso l'utilizzo di metodi come isAlive e getDeathReason
\end{itemize}
...
model (configurazione, orologio, motore di gioco, mazzo di carte),
il layer controller (comunicazione MVC e metodi di anteprima) e
le astrazioni del model layer (parametri, observer, tipi).
...
I test riguardano principalmente il sistema di carte e il caricamento dei dati da file YAML. 
Vista la brevità ho raggruppato in un'unica classe (\texttt{CardLoaderTest}) più test.
...
\paragraph{Federico Locatelli -- strategia di testing per Model, Controller e Observer}
...
da fonti esterne. La libreria utilizzata, oltre a Java~21 e JUnit~5,
è Jackson YAML per la conversione delle carte in oggetti del model.
...
\begin{itemize}
    \item \textbf{\texttt{@FunctionalInterface}} -- \texttt{ParameterObserver}
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/model/api/ParameterObserver.java#L8-L16}
    ...
    \item \textbf{Method reference come observer} -- la View viene collegata
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/controller/GameControllerImpl.java#L48-L50}

    \item \textbf{\texttt{BiConsumer} e \texttt{Function} come Strategy} --
        i tre comportamenti variabili di \texttt{CardPanel} sono rappresentati
        da interfacce funzionali iniettate dall'esterno, disaccoppiando il
        componente visuale dal Controller. \\
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/view/CardPanel.java#L7-L8}
    ...
    \item \textbf{\texttt{TextFormatter} con lambda} -- i campi di testo
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/view/LoginScene.java#L85-L93}
    ...
    \item \textbf{Switch expression Java~21} -- la selezione della difficoltà
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/view/LoginScene.java#L116}
    ...
    \item \textbf{JavaFX \texttt{Timeline} / \texttt{KeyFrame} / \texttt{KeyValue}} --
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/view/ParameterIconView.java#L150-L152}
    ...
    \item \textbf{JavaFX \texttt{ParallelTransition}} -- l'animazione di
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/view/CardPanel.java#L341}
    ...
    \item \textbf{JavaFX \texttt{BlendMode.SCREEN}} -- le icone dei parametri
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/view/ParameterIconView.java#L87-L90}

    \item \textbf{Java \texttt{record} con costruttore compatto} --
        \texttt{PlayerInfo} è dichiarato come record immutabile; il
        costruttore compatto applica la validazione fail-fast su tutti
        e tre i campi. \\
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/controller/api/PlayerInfo.java#L14}
    ...
    \item \textbf{\texttt{Platform.runLater}} -- tutti i metodi della View
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/view/GameViewJavaFXImpl.java#L322-L333}
    ...
    \item \textbf{\texttt{SVGPath} per grafica vettoriale} -- l'icona di
        ...
        \url{https://github.com/Federicolocaa/OOP25-Aurea/blob/main/src/main/java/it/unibo/aurea/view/GameViewJavaFXImpl.java#L193}
\end{itemize}
...
\paragraph{Contributi principali:}
Il mio contributo ha coperto tre livelli del sistema: la progettazione
delle astrazioni reattive del Model (\texttt{ParameterView},
\texttt{ParameterObserver}, \texttt{ParameterImpl}), l'intero layer
View con i suoi componenti animati, e l'estensione del Controller
con i metodi di anteprima. Ho infine curato il testing automatizzato.

\paragraph{Aspetti di cui sono soddisfatto:}
La scelta di rendere \texttt{ParameterObserver} una
\texttt{@FunctionalInterface} e di collegarla tramite method reference
nel Controller si è rivelata elegante e ha reso il codice molto
leggibile. Il pattern Strategy in \texttt{CardPanel} ha permesso
di mantenere il componente visuale completamente disaccoppiato dalla
logica di gioco. Sono soddisfatto anche della coerenza visiva
dell'interfaccia: il file \texttt{styles.css} unifica il tema
medievale, ma con ricordi a tema scolastico, in tutti i componenti senza ripetizioni.

\paragraph{Aspetti migliorabili e lavori futuri:}
La gestione del threading è distribuita: ogni metodo della View
chiama \texttt{Platform.runLater()} individualmente. In una versione
più matura si potrebbe centralizzare questa responsabilità in un
wrapper dedicato, riducendo la verbosità. Il foglio di stile
\texttt{styles.css}, pur funzionante, è cresciuto in modo organico
e beneficerebbe di una suddivisione in moduli tematici.

Fra i lavori futuri più interessanti: aggiungere effetti sonori
contestuali alle decisioni, introdurre animazioni sui valori numerici
dei parametri durante l'aggiornamento, e completare l'integrazione
della difficoltà nella View con feedback visivo dedicato per la
modalità \emph{Hard}.
